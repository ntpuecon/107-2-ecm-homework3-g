...
將t=0,1帶入demean後可以得到$y_{1t}-\bar{y_i}=(x_{1t}-\bar{x_i})\beta+(\epsilon_{1t}-\bar\epsilon_i)$
...
$$\bar{\bar{y}}_{t}=\frac{\sum_i\sum_t y_{it}}{NT}=\frac{\sum_{i=1}^N}{N}(\frac{\sum_{t=1}^T y_{it}}{T})=\frac{\sum_{i=1}^N(\bar{x}_{i}'\beta+c_{i}+\frac{\sum_{t=1}^T\delta_{t}}{T}+\bar\epsilon_{i})}{N}=\bar{x'}\beta+\frac{\sum_{i=1}^Nc_{i}}{N}+\frac{\sum_{i=1}^N\sum_{t=1}^T \delta_{t}}{NT}+\frac{\sum_{i=1}^N\bar\epsilon_{i}}{N}$$
...
$$\frac{\sum_{i=1}^N\sum_{t=1}^Tv_{it}\alpha_{i}}{NT}=E(v_{it}\alpha_{i})=cov(v_{it},\alpha_i)+E(v_{it})E(\alpha_i)=0$$其中因為v_{it}與\alpha_i獨立所以
cov()=0 且E(v_{it})=0
...
根據information decomposition v_{it}與\alpha_i獨立所以 $$E(v_{it}\alpha_{i})=cov(v_{it},\alpha_i)+E(v_{it})E(\alpha_i)=0$$其中因為v_{it}與\alpha_i獨立所以cov()=0 且E(v_{it})=0
...
ε_i中可能還有其他變數和x_i有關，表示ε_i和x_i不滿足條件式獨立，且ε_i會影響到y_i，則此時會有選擇性偏誤問題，故β^*的OLS估計不會正確
...
當ε_(st,i)與x_(st,i)有關時，表示可能有其他變數，例如時間效果，會影響到老人行動力(y_(st,i))，且未滿足獨立性條件，故此時的OLS估計不能代表「悠遊卡福利政策」影響。
...
當ε_(st,i)與x_(st,i)有關時，表示可能有其他變數，例如地區效果，會影響到老人行動力(y_(st,i))，且未滿足獨立性條件，故此時的OLS估計不能代表「悠遊卡福利政策」影響。
